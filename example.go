...
func ExampleUsage() {
	...
	// 3. 设置对等节点信息
	haManager.SetPeerNode("node-2", "192.168.1.101:8080", Primary)
	...
	// 5. 创建并启动API服务器
	apiServer := NewAPIServer(haManager, 8080)
...
func ExampleManualFailover(haManager *HAManager) {
...
func ExampleSyncOperations(haManager *HAManager) {
	// 立即执行同步
...
func ExampleEventOperations(haManager *HAManager) {
	// 获取事件总数
...
func ExampleDatabaseOperations(haManager *HAManager) {
	// 获取包装后的数据库连接
...
func ExampleCleanup(haManager *HAManager, apiServer *APIServer) {
